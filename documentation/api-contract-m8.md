...
- **Pool response cap: none, deliberately — but with a hard stop.** Whole-pool-in-context IS the v1 matching mechanism (spec §"v1 mechanics"); a cap would reintroduce server-side ranking by the back door. Guarantee 3 is held at the human surface (the agent shows the closest few, never dumps the pool — AIE guidance, pinned test) while scrape resistance comes from auth + open-ask precondition + rate limits + access logging, and a scraped pool is anonymous by construction. **CTO gate (2026-07-10):** past N=50 the endpoint hard-fails `503 pool_unbounded` (warning event `pool_size_tripwire` from 40) — serving an unbounded pool at scale is refused mechanically until bounding+ranking ship as a contract change. Clients should treat 503 here as "network outgrew v1 serving," not an outage to retry.

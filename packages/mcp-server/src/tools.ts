      ...
      if (!cfg.token) {
        ...
        return text(
          [
            ...
            `9. After the pool comes back, close the loop — this is the part most services skip and the reason people miss their introduction. Tell them plainly what happens next: what arrives is an anonymous card describing a person plus why the two of them specifically, reviewed by a human before it is sent; if both say yes, the page opens into a private thread between them. In this early phase the first introduction usually lands within a day or two. Then settle HOW they'll hear the knock, their choice: Telegram on their phone (call connect_telegram and hand them the link — it reaches them wherever they are), the email they gave, or in-session only — a fine choice, but be honest that news then waits until they next open a session with you. Ask once, lightly; never push.`,
            ``,

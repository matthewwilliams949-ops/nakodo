...
*(Renumber 2026-07-11, CEO ruling: **0.2.1 = copy-only integrity republish before post #1** — v2 trust copy in the shipped tool strings + version-from-package.json; already on main, Matthew publishes it in the same sitting as the Frankfurt DB handoff. The M9 train below therefore ships as **0.2.2**.)*

**M9-0 + M9a = 0.2.2 — SHIPPED to prod 2026-07-12 (backend), npm publish pending Matthew.** *(lanes: Trust & Platform `m9/feedback` @ `be74fd1` + Agent Interface `m9/agent-0.2.1` @ `cac7aa7` — NOTE: the sentiment-required blocker fix `cac7aa7` post-dates the old frozen ref `d025163`; the real verification below was run against `cac7aa7`.)* **Sequencing note: Matthew ruled 2026-07-12 to ship 0.2.2 BEFORE post #1 — first cohort lands on the better build, and migrating an empty pool is lower-risk than migrating a live one. CTO concurred; overrides the "≤48h after Day 1" gate.**
...
- [x] ~~Pre-merge blocker~~ **root-caused and FIXED (CTO, 2026-07-11): `fix/thread-ordering` @ `050827a`** — real latent bug, not test isolation: `intro_messages` ordered by `(created_at, random uuid)`, so same-microsecond messages scrambled thread order and misfired the ball-crossing nudge (the intermittent `api.test.ts:453` failure; latent on main since M8, suite passed by timing luck). Fix: insertion-ordered `seq` column + idempotent migration + regression pin (verified failing 1-in-4 unfixed, deterministic-green fixed). **Full 0.2.2 assembly (main + both M9 branches + fix) = 122 tests green ×3.** Trust review requested (their schema lane); merges with the 0.2.2 train
- [x] Ship (CTO, merge train) — done 2026-07-12, main @ `046102a` (release/0.2.2: both M9 branches + `fix/thread-ordering` + bump): re-verified 122 green against CURRENT heads (incl. `cac7aa7`, not the stale frozen ref) → **caught+fixed a prod-only migration bug**: `intro_messages_intro_seq_idx` was created in the index section BEFORE the `add column seq` ALTER — no-op on fresh DBs (tests) but "column seq does not exist" on real prod, rolled back whole apply; index creation moved after the ALTER → idempotent schema applied to Frankfurt prod (verified: `feedback` table, `seq` bigint + index, baseline intact = 1 user) → Vercel deployed (`/api/feedback` live, 401 unauth) → prod e2e via API: no-sentiment → 400 (contract fix live), valid POST → 201, body in `feedback` only, `feedback_shared` event carries moment+sentiment ONLY (guarantee-1 pin), `delete_me` → 200 cascades feedback row; throwaway user deleted, baseline re-verified clean. Also fixed: integration checkout `.env` still pointed at the dead London DB post-cutover — caught by host-check before apply, now points at Frankfurt
- [ ] **npm publish 0.2.2 — needs Matthew (security key): `cd ~/Personal/agent-networker-m8-integrate/packages/mcp-server && npm publish`.** `share_feedback` + M9a guidance live in the package, so no agent has the tool until this lands. Backend is live and verified ahead of it — publish is the only remaining step
...
**M9b — rematches / the circle** *(1–2 days · Trust & Platform + Agent Interface · retention backbone — build next after 0.2.2)*
...
- [x] AIE lane BUILT (`m9b/agent` @ fd4397a): rematch guidance (priors first, reconnection framing), `reconnect` tool (approval-gated), `rematch_proposed` client event
- [x] MERGED to main (CTO, 2026-07-12, Fable-window merge train): both lanes + gate fix (reconnect notifies the counterpart even when the reconnector spoke last — the anti-nag rule yields to a new-ask knock, or a dormant-thread reconnect is silent to an email-only counterpart)
...
**M9d tier 2 — Telegram notify** *(pulled forward per roadmap ranking — the accept-rate lever, gates #3/#4 insurance; no web surface, decoupled from M9c)*
- [x] BUILT (Activation, 2026-07-12, `m9d/telegram`, Fable-window stretch item): contract `documentation/api-contract-m9d-telegram.md` — connect via agent-handed `t.me ?start=` deep link (user's own Start tap = the approval; 30-min single-use token), `POST/DELETE /api/me/telegram`, secret-verified webhook (`/start` bind, `/stop` self-serve unbind, replies never confirm account state), exactly three DMs wired alongside email in `lib/intros.ts` (intro waiting / mutual yes / message waiting; anti-nag rule untouched), lock-screen rule test-pinned (pre-reveal = no card content, no names), best-effort sends (a dead bot never breaks the intro flow), chat-id in PII store + dies with the row (guarantee 5), `connect_telegram` MCP tool with guarantee phrasings pinned in protocol tests. 112 web + 35 MCP tests green. Feature is env-gated: all-unset = off, everything else unaffected
...
- [x] Onboarding + capture guidance rewrite: agent instructed to draft PII-free (no names/links/handles/company identifiers; city-level location only) and to treat pool content as untrusted data — done on `m8/agent-interface` (guidance aligned to T3 flag vocab, pool wrapped in explicit untrusted-data frame pinned by an injection fixture); merged + 26 MCP protocol tests green
...
- [x] Review surface: admin list of `held` proposals, one-click approve (→ proposed, notices fire) / veto (silent); script or minimal page — done 2026-07-10 on `m8/trust`: `pnpm --filter web intro:review [list|approve <id>|veto <id>]`; shows exactly the card the target would see + why_for_me; veto = new `vetoed` status, tokens dead both sides, nothing sent, cap slot freed. Also fixed in the same pass: proposing sets a_response='accepted' (the proposal IS the proposer's opt-in), expired held intros free cap slots
- [x] Tools rework: `find_collaborator` orchestrates ask → pool fetch → judge/calibrate loop ("show closest cards, ask what's off, refine") → `propose_intro` tool; guidance requires why_for_them to state what the TARGET gains — done on `m8/agent-interface` (A1–A5: pool orchestration + calibration loop + `propose_intro` + `update_my_details` PATCH-backed re-offer), merged + protocol-tested
- [x] Five guarantees v2 wording everywhere: site, intro pages, README, tool descriptions, launch posts (#2 "profile carries no identity", #3 "agents search so humans don't scroll", #4 "…and being considered is invisible too") — site + intro pages + README (P5) + tool descriptions (AIE) + welcome email (reworded, AIE review) all v2 and merged; verified live on prod nakodo.dev. **launch-posts.md v2 flip is Head of Distribution's (flagged; "concierge" → agent-driven story) — tracked in the launch handoff, not an M8 code blocker**
- [x] **E2E finding v2 (2026-07-10, escalated): the reveal stage must be a DRIVEN handoff, not a passive page.** Founder read the contact-share form as "the platform asking for my email again" (re-triggering the exact v1 trust flinch) and expected the product to connect the pair after mutual accept ("it never connected us"). Fixes: **[Lane 3 · done on `m8/product` 2026-07-10 — a–f all landed; per reveal-handoff.md; every revealed state rendered & walked; 32 web tests green]**
...
- [x] Tests: pool returns no identity fields ever (regression-pinned); propose→held→approve→standard flow; cap enforcement; lint; MCP protocol test for the new tool; pending-notice covers revealed-needs-action state — trust-lane parts done 2026-07-10 (`m8/trust`: 73 web tests incl. no-identity pin, full flow, veto/expiry silence, cap, 33 lint fixtures); MCP protocol tests on `m8/agent-interface` (21, mock-verified), pending-notice lifecycle on `m8/product` (P3) — live-wiring verified at CTO integration
- [x] Version 0.2.0; `pnpm check` green; prod migration + deploy + e2e verify (two fresh users, agent-side flow simulated over stdio) — done 2026-07-11 at CTO integration: three lanes merged (Trust `b57d371` → Product `2be852b` → AIE `df71eea`) onto `main` @ the M8 integrate commit; sole conflict (`lib/intros.ts`) resolved per pre-flight (both `introCard` form + both appended fn groups kept); **merged-tree `pnpm check` green — 85 web + 26 MCP = 111 tests, typecheck clean**; idempotent M8 migration applied to prod (verified: status check = held/vetoed/proposed/revealed/declined, a_contact/b_contact dropped, display_name/proposed_by/ask_id present); Vercel auto-deployed on push (Production ● Ready), nakodo.dev serving v2; **prod e2e via `pnpm e2e` (tooling/e2e-harness): seed → match → accept both → reveal → thread → status, both reveal pages render "this is {name}" + shared thread live on prod; all rows `source='e2e-harness'`, surgically cleaned (0 tagged rows remain), baseline otherwise unchanged.** mcp-server bumped 0.1.1 → 0.2.0. **Remaining: Matthew's live gut-check (CEO arranging) + npm publish (security key).**
